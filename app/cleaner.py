...
class CommentCleaner(object):
    ...
    def __init__(self, comments):
        ...
        self.make_sentences_and_nlp(self)
        self.tokenizeText()
        self.vectorize()
    ...
    def vectorize(self):
        count_vectorizer = pickle.load(open('models/Count_vectorizer_chicken'))
        self.all_vectorized_reviews = {}
        for key, val in self.all_tokenized_reviews.items():
            self.all_vectorized_reviews[key] = count_vectorizer.transform(val)
    ...
    def tokenizeText(self):
        ...
        for k, v in self.all_reviews.items():
            token_list = []
            for sentence in v:
                token_list.append(self.tokenizeText(sentence))
